...
public class GunGame extends JFrame{
	public GunGame() {
		...
		setContentPane(p);     // MyPanel 객체를 컨텐트팬으로 설정
		setSize(300,300);
		setResizable(false);		
		setVisible(true);
		p.startGame();
...
class MyPanel extends JPanel {
	...
	public MyPanel() {
		...
		bulletLabel.setOpaque(true);     // 배경색이 보이도록 설정
		bulletLabel.setBackground(Color.RED);
		add(baseLabel);     // baseLabel 추가
		add(targetLabel);     // targetLabel 추가
		add(bulletLabel);     // bulletLabel 추가
	...
	public void startGame() {
		baseLabel.setLocation(this.getWidth()/2-20, this.getHeight()-40);   // 패널 가운데 위치하게 설정
		bulletLabel.setLocation(this.getWidth()/2 - 5, this.getHeight()-50);  // baselabel 가운데 위치하게 설정		
	...
	class TargetThread extends Thread {
		...
		public TargetThread(JComponent target) {
			...
			target.getParent().repaint();     // target 다시 그리기
		}	
		
		@Override
		public void run() {
			while(true) {
				int x = target.getX()+5;   // target이 오른쪽으로 5씩 이동
				int y = target.getY();
		...
		public void run() {
			while(true) {
				...
				if(hit()) {
					...
					bullet.getParent().repaint();     // bullet 다시 그리기
				}
				try {
					sleep(20);
				}
